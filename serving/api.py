...
@router.get("/weather/batch/valid-weather")
def get_batch_valid_weather(
    location: Optional[str] = Query(
        None,
        description="Địa điểm cần lấy dữ liệu raw đã làm sạch/mapping.",
    ),
    province_view: Optional[str] = Query(None, description="pre_merge_63 hoặc post_merge_34"),
    start_date: Optional[date_cls] = Query(None, description="Ngày bắt đầu, định dạng YYYY-MM-DD"),
    end_date: Optional[date_cls] = Query(None, description="Ngày kết thúc, định dạng YYYY-MM-DD"),
    ...
    """Lấy dữ liệu batch raw đã làm sạch, parse timestamp và mapping tỉnh/thành từ Elasticsearch."""
    es = get_es_client()
    ...
    try:
        resp = es.search(
            index=ES_INDEX_BATCH_VALID_WEATHER,
            body={
                "size": limit,
                "query": query,
                "sort": [{"Local_Time": {"order": "desc", "unmapped_type": "date"}}],
            },
        )
    except Exception as e:
        try:
...
def get_batch_unpivoted(
    ...
    year: Optional[int] = Query(
        ...
        ge=1900,
        le=2100,
        description="Năm cần lấy dữ liệu pivot/unpivot.",
    ),
    province_view: Optional[str] = Query(POST_MERGE_VIEW, description="pre_merge_63 hoặc post_merge_34"),
    limit: int = Query(100, ge=1, le=1000, description="Số bản ghi tối đa trả về"),
) -> List[Dict[str, Any]]:
...
@router.get("/weather/ml-forecast")
def get_spark_ml_forecast(location: str = Query(..., description="Địa điểm cần lấy dự báo ML")):
    clean_location = location.replace(", VN", "").replace(", Việt Nam", "").replace(",VN", "").strip()
    es = get_es_client()

    try:
        # Truy vấn vào index weather_forecase_ml
        resp = es.search(
            index="weather_forecase_ml",
            body={
                "size": 1,
                "query": {"term": {"location.keyword": clean_location}},
                "sort": [{"grid_ts": {"order": "desc"}}],
            }
        )
        hits = resp.get("hits", {}).get("hits", [])
        if not hits:
            return {"status": "error", "message": "Chưa có dữ liệu huấn luyện ML cho khu vực này"}

        ml_data = hits[0]["_source"]
        base_time_str = ml_data.get("datetime", "")
        
        try:
            # Parse thời gian để tạo các mốc giờ (+6h, +12h, ...)
            if "T" in base_time_str:
                base_time = datetime.fromisoformat(base_time_str.replace("Z", ""))
            else:
        ...
        # Mốc 0 (Hiện tại)
        hours.append({
            "datetime": base_time.strftime("%H:%M"),
            "temp": _r(ml_data.get("current_temp")),
            "humidity": _r(ml_data.get("current_humidity")),
            "precip": _r(ml_data.get("current_precip")),
            "conditions": "Hiện tại"
        })

        # Mốc 1 đến 4 (Tương lai)
        for step in range(1, 5):
            step_time = base_time + timedelta(hours=step * 6)
            temp_key = f"forecast_temp_step_{step}"
            if temp_key in ml_data:
                precip = float(ml_data.get(f"forecast_precip_step_{step}", 0))
                hours.append({
                    "datetime": step_time.strftime("%H:%M"),
                    "temp": _r(ml_data.get(temp_key)),
                    "humidity": _r(ml_data.get(f"forecast_humidity_step_{step}")),
                    "precip": _r(precip),
                    "conditions": "Có mưa" if precip > 0 else "Tạnh ráo"
                })
                
        return {"status": "success", "data": {"hours": hours}}
    except Exception as e:
        # Xử lý lỗi an toàn nếu index chưa tồn tại
        if "index_not_found_exception" in str(e):
             return {"status": "error", "message": "Index ML chưa được tạo (Spark Inference chưa chạy xong)."}
